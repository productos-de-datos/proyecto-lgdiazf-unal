...
raw="https://raw.githubusercontent.com/jdvelasq/datalabs/master/datasets/precio_bolsa_nacional/xls/"
cwd = os.getcwd()
...
class TaskA(luigi.Task):
    ...
    def run(self):
        """
        guardar paths de los archivos descargados
        """
        lista = ingest_data.get_file_list()
        with self.output().open("w") as f:
            for archivo in lista:
                url = raw + str(archivo["path"])
...
class TaskB(luigi.Task):
    ...
    def requires(self):
        """
        toma como input los paths generados en la
        tareaA
        """
        return TaskA()
    ...
    def run(self):
        """
        Transforma los datos y genera archivo con
        los paths
        """
        ...
        with self.input().open("r") as i:
            ...
            with self.output().open("w") as f:
                for linea in lineas:
                    ...
                    path_csv = transform_data.guardar_df(
                        path, path_raw, df_transformado
                    )
                    f.write(path_csv + "\n")
...
class TaskE(luigi.Task):
    ...
    def run(self):
        with self.input()[0].open("r") as i:
            ...
            path_datos_computados = os.path.join(
                cwd, "data_lake/business/precios-mensuales.csv"
            )
